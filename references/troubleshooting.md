...
**`401 Unauthorized`**
Wrong API key. Check `ELEVENLABS_API_KEY` in `~/.viralpulse/config.json` or your environment variables.
...
**`API key invalid`**
Check `GEMINI_API_KEY` in `~/.viralpulse/config.json` or your environment variables.
...
Check `~/.viralpulse/config.json` has `topic_sources` configured. Reddit and RSS are enabled by default. Google Trends requires `pytrends`: `pip install pytrends`.
...
Drafts are saved to `~/.viralpulse/drafts/<timestamp>.json`. Check the timestamp from the draft command output.
