...
- Save the OAuth token to `~/.viralpulse/youtube_token.json`
...
```bash
python3 python -m viralpulse draft --news "your topic here"
```

On first run (no `~/.viralpulse/config.json`), the wizard will prompt for all keys and run the YouTube OAuth flow.
...
python3 python -m viralpulse produce --draft ~/.viralpulse/drafts/<id>.json

# Upload
python3 python -m viralpulse upload --draft ~/.viralpulse/drafts/<id>.json

# Full pipeline in one command
python3 python -m viralpulse run --news "your topic here"
```
...
Keys are stored in `~/.viralpulse/config.json`:
